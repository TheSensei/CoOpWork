...
*/    #if defined(ENABLE_ACK)
	...
	int freq_band = 0;
	extern int PanID[];			// declared in P2P.c
	extern int PanID_index; 	// declared in main .c file

    //First time configuration settings for MRF89XA
    ROM WORD InitConfigRegs[] = {
        /* 0 */                     GCONREG | GCONREG_SET | RF_STANDBY, 
        ...
        /* 20 */                    RSTSREG,
        /* 21 */                    OOKCREG,
        /* 22 */                    SYNCV31REG,		// removed by DWH, these are set after init routine
        /* 23 */                    SYNCV23REG,
        /* 24 */                    SYNCV15REG, 
        /* 25 */                    SYNCV07REG, 
    ...
    void RegisterSet(WORD setting)
    {
        ...
        PHY_IRQ1_En = 0;
        Config_nCS = 0;
#if defined XX || defined CO2 || defined DP1 || defined DP2
        SPIPut2((BYTE)(setting >> 8));
        SPIPut2((BYTE)setting);
#else
...
TX_END_HERE: 
    ...
    BOOL MiMAC_SetChannel(INPUT BYTE channel, INPUT BYTE offsetFreq)
    {
        ...
        // Toggle RPS values
        if (channel == CHAN_1)
			freq_band = 0x00;			// 902 band, RPS1
        else if (channel == CHAN_2)
        	freq_band = 0x09;			// 915 band, RPS2

		// Synthesize RF
        RegisterSet(GCONREG | GCONREG_SET | freq_band | RF_SYNTHESIZER);
    ...
    BOOL MiMAC_Init(INPUT MACINIT_PARAM initValue)
    {
        ...
        sync1 = PanID[PanID_index] >> 8;
        sync2 = PanID[PanID_index];
        sync3 = PanID[PanID_index + 1] >> 8;
        sync4 = PanID[PanID_index];
        RegisterSet(SYNCV31REG | sync1);
        RegisterSet(SYNCV23REG | sync2);
        RegisterSet(SYNCV15REG | sync3);
        RegisterSet(SYNCV07REG | sync4);
        SetRFMode(RF_STANDBY);
        RegisterSet(GCONREG | GCONREG_SET | freq_band | RF_SYNTHESIZER);                
        SetRFMode(RF_SYNTHESIZER);
...
void SetRFMode(BYTE mode)
{
    switch (mode) {
        case RF_TRANSMITTER:
            RegisterSet(GCONREG | GCONREG_SET | freq_band | RF_TRANSMITTER);
            RF_Mode = RF_TRANSMITTER;                //RF in TX mode
            break;
        case RF_RECEIVER:
            RegisterSet(GCONREG | GCONREG_SET | freq_band | RF_RECEIVER);
        ...
        case RF_SYNTHESIZER:
            ...
            break;
        case RF_STANDBY:
            RegisterSet(GCONREG | GCONREG_SET | freq_band | RF_STANDBY);
            RF_Mode = RF_STANDBY;                    //RF in standby mode
            break;
        case RF_SLEEP:
            RegisterSet(GCONREG | GCONREG_SET | freq_band | RF_SLEEP);
    ...
    BOOL MiMAC_ReceivedPacket(void)
    {
        ...
        for(i = 0; i < BANK_SIZE; i++)
        {
            if( RxPacket[i].flags.bits.Valid )
            {
                ...
                MACRxPacket.RSSIValue = RSSILock[i];
    ...
    void MiMAC_DiscardPacket(void)
    {
        ...
        if( ReceivedBankIndex < BANK_SIZE )
        {
		    ...
		    for (i = 0; i < RX_PACKET_SIZE; i++)		// clear current buffer row
		    {
            	RxPacket[ReceivedBankIndex].Payload[i] = 0;
      		}
            ReceivedBankIndex = 0xFF;    
        }
    }
                ...
                while(counter >= 1)
                {
                    ...
                    if(1)
                    {
                        ...
                        else
                        {
                            Data_nCS = 0;
#if defined XX || defined CO2 || defined DP1 || defined DP2
                            RxPacket[BankIndex].Payload[RxPacketPtr++] = SPIGet2();
#else
                        ...
                        if( RxPacketPtr >= PacketLen ) 
                        {
                            ...
                            #endif                                                
                            {
                                ...
                                // zero rest of payload buffer
                                for(i = PacketLen; i < RX_PACKET_SIZE; i++)
                                    RxPacket[BankIndex].Payload[i] = 0;
