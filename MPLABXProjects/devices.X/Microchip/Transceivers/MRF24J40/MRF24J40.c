...
#include "SystemProfile.h"
    ...
    void InitMRF24J40(void)
    {
        ...
        PHYSetShortRAMAddr(WRITE_TXMCR,0x80);
    }
    ...
    {   
        ...
        {
            if (RxBuffer[BankIndex].Payload[3] == 0xFF && RxBuffer[BankIndex].Payload[4] == 0xFF)
            {
            	MiMAC_DiscardPacket();
                return FALSE;   
            }
    ...
    void MiMAC_DiscardPacket(void)
    {
        ...
        if( BankIndex < BANK_SIZE )
        {
		    ...
		    {
			    RxBuffer[BankIndex].Payload[i] = 0;
	 		}
        }    
    }
    ...
    #elif defined(__PIC32MX__)
        void __ISR(_EXTERNAL_1_VECTOR, ipl4) _INT2Interrupt(void)
    #else
