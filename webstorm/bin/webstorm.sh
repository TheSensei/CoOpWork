...
if [ -n "$WEBIDE_JDK" -a -x "$WEBIDE_JDK/bin/java" ]; then
  JDK="$WEBIDE_JDK"
elif [ -s "$HOME/.WebStorm2016.3/config/webstorm.jdk" ]; then
  JDK=`"$CAT" $HOME/.WebStorm2016.3/config/webstorm.jdk`
  if [ ! -d "$JDK" ]; then
    JDK="$IDE_HOME/$JDK"
  fi
...
elif [ -r "$HOME/.WebStorm2016.3/webstorm$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.WebStorm2016.3/webstorm$BITS.vmoptions"
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,sessionname=WebStorm2016.3"
  fi
fi
...
LD_LIBRARY_PATH="$IDE_BIN_HOME:$LD_LIBRARY_PATH" "$JAVA_BIN" \
  ...
  -Didea.paths.selector=WebStorm2016.3 \
